...
class DKF(nn.Module):
    ...
    def forward(self, x0_bar, P0_bar, x1_bar, P1_bar, u1, Q1, M1, M2,  measurement_to_tag_mapping, true_pose=None, beta=0.01):
        ...
        if true_pose is not None:
            ...
            return total_loss, recon_loss, torch.tensor(0), pose_sample, x1_plus, logL

        R1_dkf = self.build_covariance_matrix(logL)
        

        # 1 Calculate regular
        kl_loss = gaussian_kl_regularization(x1_plus, R1_dkf, x1_bar, P1_bar)
        ...
        return total_loss, recon_loss, kl_loss, pose_sample, x1_hat, P1_hat, x1_plus
